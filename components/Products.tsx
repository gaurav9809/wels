...
const Products: React.FC<Props> = ({ 
  ...
  useEffect(() => {
    const loadProducts = () => {
      const p = StoreService.getProducts();
      setProducts(p);
    };
    loadProducts();
  ...
  return (
    <section id={type === 'shoe' ? 'products' : 'tshirts'} className="py-20 md:py-32 relative bg-black opacity-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center mb-12 md:mb-20">
        ...
        <div className={`grid ${getGridCols()} gap-6 md:gap-10`}>
          {filteredProducts.length > 0 ? filteredProducts.map((product) => (
            <div 
              key={product.id} 
              className="group relative glass-card p-6 md:p-8 rounded-[2.5rem] md:rounded-[3.5rem] border-white/5 hover:border-blue-500/30 transition-all duration-700 opacity-100 transform translate-y-0"
            >
            ...
            <div className="col-span-full py-32 glass-card rounded-[3rem] border-dashed border-white/10 flex flex-col items-center justify-center opacity-40">
